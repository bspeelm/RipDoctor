...
card 0: PCH [HDA Intel PCH], device 2: ALC1150 Alt Analog [ALC1150 Alt Analog]
  Subdevices: 1/1
card 3: Rx [UR23 USB SPDIF Rx], device 0: USB Audio [USB Audio]
  Subdevices: 1/1
"""
...
def test_hardware_parameters_are_read_from_stderr() -> None:
    ...
    rates, formats = V.probe(a_runner(), "hw:Rx,0")
...
def test_the_report_names_what_to_do_with_the_answer() -> None:
    ...
    assert "hw:Rx,0" in text and "capture_device" in text
    assert "96000" in text and "S24_3LE" in text


def test_the_configured_device_is_marked() -> None:
    text = V.report(V.enumerate_devices(a_runner()), configured="hw:Rx,0")
    assert "* hw:Rx,0" in text
    assert "configured: hw:Rx,0" in text
