...
def creds(user: str = "abbey", password: str = "secret") -> A.Credentials:
...
def test_the_right_password_verifies() -> None:
    assert creds().verify("abbey", "secret")


@pytest.mark.parametrize(
    ("user", "password"), [("abbey", "wrong"), ("other", "secret"), ("", "")]
)
...
def test_a_file_that_is_not_credentials_is_reported_clearly(tmp_path: Path) -> None:
    target = tmp_path / "auth.json"
    target.write_text(json.dumps({"user": "abbey"}))
...
def test_a_first_run_generates_a_password_and_returns_it_once(
    ...
    target = tmp_path / "auth.json"
    c, generated = A.load_or_create(target, "abbey", iterations=FAST)
    assert generated and len(generated) > 12
    assert c.verify("abbey", generated)
...
def test_a_second_run_reads_what_is_there_rather_than_replacing_it(
    ...
    target = tmp_path / "auth.json"
    first, _ = A.load_or_create(target, "abbey", iterations=FAST)
    again, generated = A.load_or_create(target, "abbey", iterations=FAST)
...
def test_a_supplied_password_is_used_and_not_reported(tmp_path: Path) -> None:
    c, generated = A.load_or_create(
        tmp_path / "a.json", "abbey", "mine", iterations=FAST
    )
    assert generated is None and c.verify("abbey", "mine")
...
def test_a_session_names_its_user() -> None:
    ...
    assert s.check(s.issue("abbey", now=clock(1000.0)), now=clock(1000.0)) == "abbey"


def test_a_session_expires() -> None:
    s = A.Sessions(secret=b"0" * 32, ttl=60.0)
    token = s.issue("abbey", now=clock(1000.0))
    assert s.check(token, now=clock(1059.0)) == "abbey"
...
def test_two_sessions_for_one_user_differ() -> None:
    ...
    s = A.Sessions(secret=b"0" * 32)
    assert s.issue("abbey", now=clock(1.0)) != s.issue("abbey", now=clock(1.0))
