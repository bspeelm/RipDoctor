"""Deciding when a side has ended, without a clock. docs/method.md."""
...
def step(
    ...
    *,
    ...
    if music < ARM_FLOOR:
        # Never arm here: cueing a needle would trip it immediately.
        warning = None
        if elapsed > FLOOR_WARN_AFTER:
            # Past thirty seconds this is a wrong input, a dead cable, or an
            # arm still in its rest. One capture ran its whole length that way.
            warning = (
...
def run(
    ...
    """Fold a whole capture. Returns (state, stop reason, elapsed at stop)."""
    state = State()
